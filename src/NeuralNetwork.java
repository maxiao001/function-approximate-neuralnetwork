...
public class NeuralNetwork {
	...
	int init(List<Integer> layer_units_list,
		...
		System.out.println("total par num:"+total_par_num);
	...
	private void random_par() {
		
		for(int i = 0;i < par.length;i++){
			par[i] = ((double)r.nextInt(1000)/1000-0.5)/par.length;
		}
	...
	int back_propagation(double[] last_layer_grad) {
		...
		if (last_layer_grad.length != layer_units[layer_num - 1]) {
			System.out
		...
		for (int l = layer_num - 2; l >= 0; l--) {
			...
			for (int i = 0; i < layer_units[l]; i++) {
				...
				for (int j = 0; j < layer_units[l + 1]; j++) {
					sigma_buf[l_unit_disp + i] += par[l_w_disp + j * layer_units[l] + i]
				...
				for (int j = 0; j < layer_units[l]; j++) {
					grad[l_w_disp + i * layer_units[l] + j] += a_buf[l_unit_disp
							...
							* sigma_buf[ln_unit_disp + i]
							+ regular_lambda
							* par[l_w_disp + i * layer_units[l] + j];
	...
	int update_par(double learning_rate) {

		for (int i = 0; i < par.length; i++) {
			par[i] -= learning_rate * grad[i];
		}
		return 0;
	}
